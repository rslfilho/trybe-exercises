...
// Função do Bootsrap para validação. Editada para chamar a criação do currículo na tela.
...
function createNameP(inputIndex) {
  ...
  dataField.classList.add('text-muted');
...
function createLabel(labelIndex) {
  ...
  const personalData = document.getElementById('personal');
  const lastJobData = document.getElementById('last-job');
  const labelField = document.createElement('h6');
...
function removeCreatedElement() {
  const parentElement = document.querySelector('body');
  const nextElement = document.querySelector('main');
  
  if (nextElement !== parentElement.firstElementChild.nextElementSibling) {
    parentElement.removeChild(parentElement.firstElementChild.nextElementSibling);
  }
}
